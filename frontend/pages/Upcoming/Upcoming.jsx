...
const Upcoming = () => {
  ...
  useEffect(() => {
    async function apicall() {
      try {
        setLoading(true);
        const response = await axios.get(`seasons/upcoming?limit=24&page=${page}`);
                  ...
                  <p className='popular-card-genres'>
                    Release Date : {
                      anime.aired?.prop?.from?.day && anime.aired?.prop?.from?.month && anime.aired?.prop?.from?.year
                        ? `${anime.aired.prop.from.day} ${getmonths(anime.aired.prop.from.month)} ${anime.aired.prop.from.year}`
                        : "Releasing soon"
                    }
                  </p>
